...
def load_data(fpath='/var/tmp/mnist.npz', rescale=True, xshape=None,
              onehot=True):
    ...
    if xshape:
        ...
        X_test = np.reshape(X_test, xshape)
...
def _extract_images(fpath):
    with gzip.GzipFile(fpath) as bytestream:
        ...
        if magic != 2051:
            raise ValueError('Invalid magic number {0} in MNIST image file:'
                             ' {1}'.format(magic, fpath))
...
def _extract_labels(fpath):
    with gzip.GzipFile(fpath) as bytestream:
        magic = _read32(bytestream)
        if magic != 2049:
            raise ValueError('Invalid magic number {0} in MNIST label file:'
                             ' {1}'.format(magic, fpath))
